...
How this site actually implements the Cohere design system. These values are the source of truth for `static/css/global.css`.

### Font Substitutions

Cohere's proprietary fonts are unavailable. This site uses:

| Role | Cohere original | This site |
|---|---|---|
| Display headlines | CohereText | Space Grotesk (Google Fonts, wght 300–700) |
| Body / UI | Unica77 Cohere Web | Inter (Google Fonts, wght 400 & 500) |
| Mono labels | CohereMono | `ui-monospace, 'Courier New', monospace` |

Loaded via `@import` in CSS (no `<link>` tag in HTML).

### Scaled Type System

The site runs at ~1.875× the base Cohere spec (150% zoom feel, then +25%). All sizes below are the actual rendered values.

| Role | CSS class | Size | Weight | Letter Spacing |
|---|---|---:|---:|---:|
| Home hero | `.home-hero-title` | 100px | 400 | -2px |
| Post / page title | `.post-title`, `.page-title`, `.archive-title` | 80px | 400 | -0.8px |
| Post content h2 | `.post-content h2` | 48px | 400 | -0.48px |
| Post content h3 | `.post-content h3` | 38px | 400 | -0.38px |
| Home post title | `.home-post-title` | 35px | 400 | -0.35px |
| Body | `body` | 25px | 400 | — |
| Hero subtitle | `.home-hero-subtitle` | 28px | 400 | — |
| Nav link | `.nav-link` | 21px | 400 | — |
| Section label (mono) | `.section-label` | 18px | 400 | 0.28px |
| Archive date | `.archive-post-date` | 18px | 400 | 0.14px |
...
- **Container**: `max-width: 1250px`, `padding: 0 45px 150px` (`0 25px 100px` on mobile)
- **Header**: `padding: 40px 0 45px`, `margin-bottom: 100px` (72px at ≤1100px)
- **Footer**: full-bleed `site-footer-wrap` div, inner container matches page max-width

### Logo / Brand Mark

Photo (`/chancesmith_s.png`) with coral double-ring via `box-shadow`:
...
.site-logo {
  ...
  box-shadow: 0 0 0 2px var(--c-canvas), 0 0 0 4px var(--c-coral);
}

/* Homepage only (.site-brand--home) */
.site-brand--home .site-logo {
  width: 110px; height: 110px;
  box-shadow: 0 0 0 3px var(--c-canvas), 0 0 0 6px var(--c-coral);
}
```

Brand name "Chance Smith" hides at ≤700px to prevent nav overflow.

### Dark Mode

Toggled via `data-theme="dark"` on `<html>`. Persisted in `localStorage`. FOUC-prevention inline script runs before CSS loads.
...
[data-theme="dark"] {
  --c-canvas: #111113;
  --c-ink: #e8e8ec;
  --c-soft-stone: #1a1a1f;   /* footer stays dark */
  --c-hairline: #28282e;
  --c-action-blue: #5b9cf6;
}
```

Footer background uses `var(--c-soft-stone)` so it automatically adapts without extra selectors.

### Breakpoints

| Width | Key changes |
|---:|---|
| ≤1100px | Header margin 72px; hero 72px; titles 60px; archive rows stack; footer stacks |
| ≤700px | Shell padding 25px; brand name hidden; nav gap 20px; hero 42px; titles 36px with `overflow-wrap: break-word` |

### Layout Stability

`html { scrollbar-gutter: stable; }` prevents content reflow on pages with/without scrollbars (e.g. about page vs. long posts).
